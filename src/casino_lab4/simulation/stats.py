...
def take_snapshot(casino: Casino) -> SimulationSnapshot:
    ...
    geese = {g.name: (float(g.balance), type(g).__name__) for g in casino.geese}

    return SimulationSnapshot(
        players=players, geese=geese, casino_bankroll=float(casino.bankroll)
    )


def compute_stats(
    initial: SimulationSnapshot, final: SimulationSnapshot
) -> tuple[list[PlayerStats], list[GooseStats], CasinoStats]:
    player_stats = []
...
def print_statistics(
    ...
    logger.info(
        f"  Status:           {'BANKRUPT' if casino_stats.is_bankrupt else 'OPERATING'}"
    )
    ...
    for ps in player_stats:
        ...
        logger.info(
            f"    Balance: ${ps.initial_balance:,.2f} -> ${ps.final_balance:,.2f} ({ps.balance_change:+,.2f})"
        )
        logger.info(
            f"    Sanity:  {ps.initial_sanity} -> {ps.final_sanity} ({ps.sanity_change:+d})"
        )
    ...
    for gs in goose_stats:
        ...
        logger.info(
            f"    Balance: ${gs.initial_balance:,.2f} -> ${gs.final_balance:,.2f} ({gs.balance_change:+,.2f})"
        )
    ...
    logger.info(
        f"  Casino Status: {'BANKRUPT' if casino_stats.is_bankrupt else 'OPERATING'}"
    )
